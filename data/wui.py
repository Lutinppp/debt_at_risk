...
def _build_synthetic_wui() -> pd.DataFrame:
    """
    Build a synthetic WUI panel when the data file is unavailable.
    ...
    global_wui = {
        ...
        2020: 0.90, 2021: 0.55, 2022: 0.60, 2023: 0.40, 2024: 0.35, 2025: 0.32,
    }
    ...
    for iso3 in countries:
        for year in range(1990, 2026):
            ...
            records.append({"iso3": iso3, "year": year, "wui": max(0.0, base + noise)})
...
def load_wui() -> pd.DataFrame:
    """Load cached WUI data; re-fetch if not present."""
    if WUI_CACHE.exists():
        return pd.read_parquet(WUI_CACHE)
